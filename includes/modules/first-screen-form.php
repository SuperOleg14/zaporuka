...
if (!$active_project_id) {
    ...
    if ($query->have_posts()) {
        $query->the_post();
        $active_project_id = get_the_ID();
        wp_reset_postdata();
    }
}
?>

<div class="donation-container grey-block short-form">
    <?php if (!is_page_template('pages/page-regular-donations.php')) : ?>
        <div class="donation-switch">
            <span class="text first">Разова допомога</span>
            <label for="donation-toggle-main" class="donation-switch-label">
                <input type="checkbox" id="donation-toggle-main" class="donation-toggle">
    ...
    <div class="block-subscribe block-donation-text">
        <?php if (is_page_template('pages/page-regular-donations.php')) : ?>
            <div class="title">
                Оформіть підписку та долучайтесь до нашої спільноти!
            </div>
        <?php else : ?>
            <div class="title">
    ...
    <form id="donation-form-main-page" class="donation-form">
        <div class="block-input">
            <label for="amount-main">
                <input type="number" id="amount-main" placeholder="Ваш внесок" class="input-amount" required>
            </label>
        ...
        <div class="inputs-grid column-1 hide">
        ...
        <input type="hidden" name="page_id" value="<?php echo esc_attr($active_project_id); ?>">

        <button type="submit" class="form-btn btn submit-donate">
            <?php if (is_page_template('pages/page-regular-donations.php')) : ?>
                Підтримати за підпискою
            <?php else : ?>
                Підтримати
            <?php endif; ?>
