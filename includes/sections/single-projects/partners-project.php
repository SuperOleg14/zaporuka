<div class="our-partners m-150">
    <div class="container">
        <h2 class="title --text-align">
            <?php echo get_sub_field( 'partners_project_title'); ?>
        </h2>
        <div class="our-partners__content d-flex --just-space --align-stretch f-wrap">
            <?php if ( have_rows( 'partners_project_content' ) ): ?>
                <?php while ( have_rows( 'partners_project_content' ) ) : the_row(); ?>
                    <div class="our-partners__content_item d-flex --jc-center --align-center">
                        <img src="<?php echo get_sub_field('partners_project_content_photo') ?>"/>
                    </div>
                <?php endwhile; ?>
            <?php endif; ?>
        </div>
        <?php $partnersProjectLink = get_sub_field('partners_project_link'); ?>
        <?php if ( $partnersProjectLink ): ?>
            <a href="<?php echo esc_url( $partnersProjectLink['url']) ?>" class="btn btn-content --content-margin" target="_blank">
                <?php echo esc_html( $partnersProjectLink['title']) ?>
